...
struct GLNVGfragUniforms {
		...
		// don't forget to also update the fragment mShader source!
...
#else
	mShader->loc[GLNVG_LOC_FRAG] = glGetUniformLocation(mShader->prog, "frag");
#endif
}
...
static void glnvg__setUniforms(GLNVGcontext* gl, int uniformOffset, int image)
{
	...
	glUniform4fv(gl->mShader.loc[GLNVG_LOC_FRAG], NANOVG_GL_UNIFORMARRAY_SIZE, &(frag->uniformArray[0][0]));
#endif
							  ...
							  const float* bounds, const NVGpath* paths, int npaths)
{
	...
	// Allocate mvVertices for all the paths.
