...
// Tip: use ImGui::PushID()/PopID() to push mvIndices or pointers in the ID stack.
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // For logarithmic use our range is effectively 0..1 so scale the f_Delta into that range
    if (is_logarithmic && (v_max - v_min < FLT_MAX) && ((v_max - v_min) > 0.000001f)) // Epsilon to avoid /0
        adjust_delta /= (float)(v_max - v_min);

    // Clear current value on activation
    // Avoid altering values and clamping when we are _already_ past the limits and yaw in the same direction, so e.g. if range is 0..255, current value is 300 and we are pushing to the right side, keep the 300.
    ...
    if (is_logarithmic)
    {
        ...
        // Convert to parametric space, apply f_Delta, convert back
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Keyboard || g.ActiveIdSource == ImGuiInputSource_Gamepad)
        {
            if (g.ActiveIdIsJustActivated)
            {
                g.SliderCurrentAccum = 0.0f; // Reset any stored nav f_Delta upon activation
            ...
            float f_Delta = g.SliderCurrentAccum;
            ...
            else if (g.SliderCurrentAccumDirty)
            {
                ...
                if ((clicked_t >= 1.0f && f_Delta > 0.0f) || (clicked_t <= 0.0f && f_Delta < 0.0f)) // This is to avoid applying the saturation when already past the limits
                {
                    set_new_value = false;
                ...
                else
                {
                    ...
                    clicked_t = ImSaturate(clicked_t + f_Delta);
                    ...
                    if (f_Delta > 0)
                        g.SliderCurrentAccum -= ImMin(new_clicked_t - old_clicked_t, f_Delta);
                    else
                        g.SliderCurrentAccum -= ImMax(new_clicked_t - old_clicked_t, f_Delta);
                }
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // without any carriage return, which would makes ImFont::RenderText() reserve too many mvVertices and probably crash. Avoid it altogether.
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        for (int n = 0; n < 6; n++)
        {
            ...
            // Paint colors over existing mvVertices
...
//   if your SetNextItemSelectionUserData() values are mvIndices, you can obtain it from ImGuiMultiSelectIO::NavIdItem, otherwise from g.NavLastValidSelectionUserData.
...
//   - In this demo we often submit mvIndices to SetNextItemSelectionUserData() + store the same mvIndices in persistent selection.
...
// - A full-featured application is likely to allow search/filtering which is likely to lead to using mvIndices
...
// Notice that with the simplest adapter (using mvIndices everywhere), all functions return their parameters.
// The most simple implementation (using mvIndices everywhere) would look like:
