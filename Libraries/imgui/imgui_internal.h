...
enum ImGuiWindowRefreshFlags_
{
    ...
    // Refresh policy/f_Frequency, Load Balancing etc.
};
...
struct ImGuiListClipperRange
{
    int     Min;
    int     Max;
    bool    PosToIndexConvert;      // Begin/End are absolute position (will be converted to mvIndices later)
    ImS8    PosToIndexOffsetMin;    // Add to Min after converting to mvIndices
    ImS8    PosToIndexOffsetMax;    // Add to Min after converting to mvIndices
...
// We always assume that -1 is an invalid value (which works for mvIndices and pointers)
...
struct ImGuiContext
{
    ...
    float                   SliderCurrentAccum;                 // Accumulated slider f_Delta when using navigation controls.
    bool                    SliderCurrentAccumDirty;            // Has the accumulated slider f_Delta changed since last time we tried to apply it?
...
//#endif
    ...
    // Shade functions (write over already created mvVertices)
